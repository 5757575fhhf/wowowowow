...
function makeField(){
 ...
 for(let i=0;i<19;i++){
  ...
  let teamTier=i%cur().teams.length,teamPerf=cur().teams[teamTier][1];
...
function ensureWorldSeries(){if(S.worldSeries&&S.worldSeries.season===S.season)return;S.worldSeries={season:S.season,divisions:{}};for(let si=1;si<=4;si++){let cfg=SERIES[si],teams=cfg.teams.map((t,ti)=>({name:t[0],points:0,perf:t[1],budget:100,factory:1})),drivers=[];for(let i=0;i<20;i++){let ti=i%cfg.teams.length,n=FIRST[(i*5+si*3)%FIRST.length]+' '+LAST[(i*7+si*2)%LAST.length];drivers.push({id:`s${si}d${i}`,name:n,team:ti,points:0,skill:clamp(48+si*8+rnd(-12,12),35,98)})}S.worldSeries.divisions[si]={round:0,drivers,teams}}}
...
function upgrade(k){ensureFactory();let part=DEV_PARTS[k];if(!part)return;let level=devVal(k),base=Math.round((10+S.series*7+level*3.5)*part.cost),cost=Math.max(1,Math.round(base*factoryCostMultiplier()));if(costCapSpend()+cost>costCap())return notice('This development would exceed the team budget cap.');if(!teamBudgetSpend(cost,part.name+' development'))return;let gain=1.5*factoryEfficiency(k),setback=Math.random()<factoryFailureChance();if(setback){gain*=.35;notice(part.name+' R&D hit a setback.');hist('⚠️ '+part.name+' R&D setback')}else notice(part.name+' developed +'+gain.toFixed(1)+' with factory support.');S.dev[k]=clamp(level+gain,0,16);hist('🔧 '+part.name+' upgraded');render();save()}
