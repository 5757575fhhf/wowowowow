...
function finishRace(){
 ...
 S.rep=clamp(S.rep+(dnf?-1:Math.max(-.5,6-finish*.28)),0,100);academyPerformanceUpdate(finish,dnf);academyStandingUpdate(finish,dnf);if(!r.sprint)maybePoach();S.cash+=3+S.series*6+(finish<=10?11-finish:0);if(!r.sprint){awardSuperLicence(finish);updateSponsorObjectives(finish,dnf);simulateOtherDivisions();}
...
function upgrade(k){let part=DEV_PARTS[k];if(!part)return;let level=devVal(k),cost=Math.round((10+S.series*7+level*3.5)*part.cost);if(S.cash<cost)return notice('Not enough cash for '+part.name+' upgrade.');S.cash-=cost;S.dev[k]=clamp(level+1.5,0,16);notice(part.name+' development completed.');hist('🔧 '+part.name+' upgraded');render();save()}
