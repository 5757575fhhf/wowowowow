...
function finishRace(){
 ...
 S.rep=clamp(S.rep+(dnf?-1:Math.max(-.5,6-finish*.28)),0,100);academyPerformanceUpdate(finish,dnf);academyStandingUpdate(finish,dnf);if(!r.sprint)maybePoach();S.cash+=3+S.series*6+(finish<=10?11-finish:0);if(!r.sprint){awardSuperLicence(finish);try{updateSponsorObjectives(finish,dnf)}catch(e){console.error('Sponsor objective error',e)}
...
function ensureWorldSeries(){if(S.worldSeries&&S.worldSeries.season===S.season)return;S.worldSeries={season:S.season,divisions:{}};for(let si=1;si<=4;si++){let cfg=SERIES[si],teams=cfg.teams.map((t,ti)=>({name:t[0],points:0,perf:t[1]})),drivers=[];for(let i=0;i<20;i++){let ti=i%3,n=FIRST[(i*5+si*3)%FIRST.length]+' '+LAST[(i*7+si*2)%LAST.length];drivers.push({id:`s${si}d${i}`,name:n,team:ti,points:0,skill:clamp(48+si*8+rnd(-12,12),35,98)})}S.worldSeries.divisions[si]={round:0,drivers,teams}}}
function simulateOtherDivisions(){ensureWorldSeries();for(let si=1;si<=4;si++){if(si===S.series)continue;let d=S.worldSeries.divisions[si];if(!d||d.round>=SERIES[si].rounds)continue;let order=[...d.drivers].sort((a,b)=>(b.skill+d.teams[b.team].perf*.45+rnd(-9,9))-(a.skill+d.teams[a.team].perf*.45+rnd(-9,9)));order.slice(0,10).forEach((x,i)=>{x.points+=POINTS[i]||0;d.teams[x.team].points+=POINTS[i]||0});d.round++}}
function currentConstructors(){let rows=cur().teams.map((t,ti)=>({name:t[0],points:0,ti}));rows[S.teamTier].points+=S.points||0;S.field.forEach(d=>rows[d.teamTier??0].points+=(S.standings[d.id]||0));return rows.sort((a,b)=>b.points-a.points)}
function renderWorldStandings(){let box=$('worldStandings');if(!box)return;ensureWorldSeries();box.innerHTML=[1,2,3,4].map(si=>{if(si===S.series){return `<div class="divisionCard"><h3>${SERIES[si].name}<span>YOUR SERIES</span></h3>${standingRows().slice(0,10).map((d,i)=>`<div class="miniStanding ${d.player?'you':''}"><b>${i+1}</b><span>${d.name}</span><strong>${d.points}</strong></div>`).join('')}</div>`}let d=S.worldSeries.divisions[si],rows=[...d.drivers].sort((a,b)=>b.points-a.points).slice(0,10);return `<div class="divisionCard"><h3>${SERIES[si].name}</h3><small>Round ${d.round}/${SERIES[si].rounds}</small>${rows.map((x,i)=>`<div class="miniStanding"><b>${i+1}</b><span>${x.name}</span><strong>${x.points}</strong></div>`).join('')}</div>`}).join('');let cb=$('constructorsTable');if(cb)cb.innerHTML='<table><tr><th>Pos</th><th>Constructor</th><th>Pts</th></tr>'+currentConstructors().map((x,i)=>`<tr><td>${i+1}</td><td>${x.name}</td><td>${Math.round(x.points)}</td></tr>`).join('')+'</table>'}
...
function upgrade(k){ensureFactory();let part=DEV_PARTS[k];if(!part)return;let level=devVal(k),base=Math.round((10+S.series*7+level*3.5)*part.cost),cost=Math.max(1,Math.round(base*factoryCostMultiplier()));if(S.cash<cost)return notice('Not enough cash for '+part.name+' upgrade.');S.cash-=cost;let gain=1.5*factoryEfficiency(k),setback=Math.random()<factoryFailureChance();if(setback){gain*=.35;notice(part.name+' R&D hit a setback.');hist('⚠️ '+part.name+' R&D setback')}else notice(part.name+' developed +'+gain.toFixed(1)+' with factory support.');S.dev[k]=clamp(level+gain,0,16);hist('🔧 '+part.name+' upgraded');render();save()}
...
document.addEventListener('change',e=>{if(e.target?.id==='preseasonFocus'&&S.preseason){S.preseason.focus=e.target.value;renderPreseason();save()}});
ensure();
