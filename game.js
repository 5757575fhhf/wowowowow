...
function ensure(){
 if(S.series>=1&&!S.constructorExpansionV101){S.constructorExpansionV101=true;S.teamTier=clamp(S.teamTier||0,0,cur().teams.length-1);makeField();S.worldSeries=null;}
...
function renderRaceReview(){
 ...
 ${S.lastRaceRating?`<div class="raceRating"><strong>${S.lastRaceRating.rating}/10</strong><span>Driver rating • ${S.lastRaceRating.overtakes} overtakes • expected P${S.lastRaceRating.expected}</span></div>`:''}<div class="reviewText"><p><b>Strategy:</b> ${a.strategy}</p><p><b>Pit review:</b> ${a.pits}</p><p><b>Tyres:</b> ${a.tyres}</p><p><b>Race incidents:</b> ${incidents} retirement${incidents===1?'':'s'} in the field.</p></div>`;
;targets.slice(1).forEach(x=>x.innerHTML=el.innerHTML)}
